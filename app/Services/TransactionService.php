...
class TransactionService {
    function sell(Request $request){
        ...
        $sell->transaction_date = $request->input('transaction_date');
        $sell->supplier_name = $request->input('customer_name');

        echo $request->input . "\n";

        if(array_key_exists('is_cancelled', $request->input)){
        ...
        if(array_key_exists('is_print', $request->input)){
            $sell->is_cancelled = $request->input('is_cancelled');
            if($request->input('is_cancelled') == true){
                $sell->cancelled_at = $request->input('cancelled_at');
            }
        ...
        foreach($details as $d){
            $sellDetail = new PurchaseTransactionDetail;
            $sellDetail->purchase_transaction_id = $sell->ID;
            $sellDetail->raw_material_id = $d['raw_material_id'];
            ...
            $sellDetail->total = (($d['qty'] * $d['price']) * $multiplier);
